...
function validateProfileName(value) {
  if (!PROFILE_NAME_PATTERN.test(value)) {
    throw new SLAError("Profile names may only contain letters, numbers, dot, underscore, and hyphen.", {
...
function validateSkillName(value) {
  if (!SKILL_NAME_PATTERN.test(value)) {
    throw new SLAError("Skill names may only contain letters, numbers, dot, underscore, and hyphen.", {
...
function validateMemoryTarget(value) {
  if (!MEMORY_TARGETS.has(value)) {
    throw new SLAError("Memory target must be 'memory' or 'user'.", {
...
function validateSkillSubdir(value) {
  if (!SKILL_SUBDIRS.has(value)) {
    throw new SLAError("Managed skill subdirectory must be one of references, templates, scripts, or assets.", {
...
function validateRelativeManagedPath(value) {
  if (!value || path.isAbsolute(value)) {
    throw new SLAError("Managed file paths must be relative.", {
